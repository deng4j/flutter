...
Flutter中⼏乎所有的对象都是⼀个Widget，与原⽣开发中“控件”不同的是，Flutter中的widget的概念更⼴泛，它不仅可以表示UI元素，也可以表示⼀些功能性的组件如：⽤于⼿势检测的 GestureDetector widget、⽤于应⽤主题数据传递的 Theme 等等。⽽原⽣开发中的控件通常只是指UI元素。

由于Flutter主要就是⽤于构建⽤户界⾯的，所以，在⼤多数时候，读者可以认为widget就是⼀个控件，不必纠结于概念。 
...
## 2.用MaterialApp和Scaffold两个组件装饰App
...
# 十四.Widget Tree、Element Tree 和 RenderObject Tree
...
# 十五. AnimatedList增加列表FadeTransition、ScaleTransition
