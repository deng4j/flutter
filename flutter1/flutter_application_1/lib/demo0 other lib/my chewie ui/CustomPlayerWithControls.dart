...
///自定义播放器UI
class CustomPlayerWithControls extends StatefulWidget {
  //初始化时是否展示loading
  final bool showLoadingOnInitialize = true;

  //是否展示大播放按钮
  final bool showBigPlayIcon = true;

  //视频浮层
  final Widget overlayUI = Center();
  ...
  //弹幕浮层
  final Widget barrageUI = Center();

  CustomPlayerWithControls({super.key});

  @override
  State<StatefulWidget> createState() {
    return _CustomPlayerWithControlsState();
  }
}

class _CustomPlayerWithControlsState extends State<CustomPlayerWithControls>
    with SingleTickerProviderStateMixin {
  late VideoPlayerValue _latestValue;
  late double _latestVolume = 1.0;
  bool _hideStuff = true;
  late Timer _hideTimer = Timer(Duration(seconds: 2), () {
    print(DateTime.now().toString());
  });
  late Timer _initTimer = Timer(Duration(seconds: 2), () {
    print(DateTime.now().toString());
  });
  late Timer _showAfterExpandCollapseTimer = Timer(Duration(seconds: 2), () {
    print(DateTime.now().toString());
  });
